...
public class AdminBooksFromController {
    ...
    void btnSaveOnAction(ActionEvent event) {
        ...
        if (combBranch.getValue().isEmpty() || textAuthor.getText().isEmpty() || textStatus.getText().isEmpty()
                || textGen.getText().isEmpty() || textTitle.getText().isEmpty()) {
            ...
            if (isValid || isValid1 || isValid2 || isValid3 || isValid4) {
                ...
                if (btnSave.getText().equals("Save")) {
                    ...
                    long bookId = Long.parseLong(lblBookId.getText());
                    boolean isUpdate = bookBO
                            .updateBook(new BookDTO(bookId, title, author, availabilityS, gen, branchDTO));
                    if (isUpdate) {
    ...
    public void loadAllBooks() {
        ...
        for (BookDTO dto : bookDTOS) {
            ...
            tblViwe.getItems().add(new BookTM(dto.getId(), dto.getTitle(), dto.getAuthor(), dto.getAvailabilityStatus(),
                    dto.getGeneration(), branchDTO.getAddress()));
        }
    }
